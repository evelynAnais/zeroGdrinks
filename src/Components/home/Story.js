      ...
      <Card>
        <Card.Body>
          ...
          <p>On a { randomWord(weatherRandom) } { randomWord(timeOfDayRandom) } in a galaxy called 
            ...
            and {story?.number} showed up. It was a { randomWord(partyRandom) }. They drank { drink?.strDrink } and danced to { story?.artist }.
            They were having so much fun that they attracted some { randomWord(unexpectedRandom) } guests! Aliens, { randomWord(sizeRandom) } { story?.color } aliens
            with { randomWord(sizesRandom) } {story?.bodyPart} who ended up being the life of the party. { story.friend } gave { story?.idol } the alien
            a { drink?.strDrink } and a {story?.object}. They were having so much fun! They partied for { randomWord(timeRandom) }. Everyone 
            decided they needed to make it a tradition to celebrate how { randomWord(amazingRandom) } life was on {spaceObject?.name} with their new alien friends! So 
            every {story?.number} { randomWord(timesRandom) } they celebrate with { drink?.strDrink } and { story?.idol }!
          </p>
        </Card.Body>
      </Card>
    </div>
